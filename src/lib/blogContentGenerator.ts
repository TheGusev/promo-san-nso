...
const introTemplates = {
  pests: (topic: BlogTopic) => `
    <p class="lead">
      ...
      В этой статье эксперты СЭС «СанРешения» подробно разберут ${topic.description.toLowerCase()}.
    </p>
    <p>
  ...
  disinfection: (topic: BlogTopic) => `
    ...
    <p>
      СЭС «СанРешения» проводит дезинфекцию в Новосибирске с использованием сертифицированных 
  ...
  faq: (topic: BlogTopic) => `
    ...
    <p>
      Мы собрали ответы на самые частые вопросы клиентов СЭС «СанРешения» в Новосибирске и 
...
function generateSectionContent(heading: string, topic: BlogTopic): string {
  ...
  const templates = [
    ...
    <p>
      По опыту специалистов СЭС «СанРешения», большинство ошибок при самостоятельной обработке связаны 
    ...
    <p>
      Профессиональный подход СЭС «СанРешения» включает несколько ключевых этапов:
    </p>
    <ol>
    ...
    <blockquote>
      <p>
        ...
        проблемы даёт только временный эффект», — главный специалист СЭС «СанРешения».
      </p>
    </blockquote>`,
    
    `<p>
      В контексте <em>${heading.toLowerCase()}</em> важно понимать несколько ключевых моментов. 
      Опыт работы СЭС «СанРешения» в Новосибирске показывает типичные ошибки, которых можно избежать.
    </p>
...
function generateFAQ(topic: BlogTopic): FAQItem[] {
  const baseFAQ: FAQItem[] = [
    ...
    },
    {
      ...
      answer: `СЭС «СанРешения» предоставляет гарантию на все виды обработок. Срок гарантии — от 6 месяцев 
