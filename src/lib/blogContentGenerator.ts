...
const introTemplates = {
  pests: (topic: BlogTopic) => `
    <p class="lead">
      ...
      В этой статье эксперты СЭС «Гордез» подробно разберут ${topic.description.toLowerCase()}.
    </p>
    <p>
  ...
  disinfection: (topic: BlogTopic) => `
    ...
    <p>
      СЭС «Гордез» проводит дезинфекцию в Новосибирске с использованием сертифицированных 
  ...
  faq: (topic: BlogTopic) => `
    ...
    <p>
      Мы собрали ответы на самые частые вопросы клиентов СЭС «Гордез» в Новосибирске и 
...
function generateSectionContent(heading: string, topic: BlogTopic): string {
  ...
  const templates = [
    ...
    <p>
      По опыту специалистов СЭС «Гордез», большинство ошибок при самостоятельной обработке связаны 
    ...
    <p>
      Профессиональный подход СЭС «Гордез» включает несколько ключевых этапов:
    </p>
    <ol>
    ...
    <blockquote>
      <p>
        ...
        проблемы даёт только временный эффект», — главный специалист СЭС «Гордез».
      </p>
    </blockquote>`,
    
    `<p>
      В контексте <em>${heading.toLowerCase()}</em> важно понимать несколько ключевых моментов. 
      Опыт работы СЭС «Гордез» в Новосибирске показывает типичные ошибки, которых можно избежать.
    </p>
...
function generateFAQ(topic: BlogTopic): FAQItem[] {
  const baseFAQ: FAQItem[] = [
    ...
    },
    {
      ...
      answer: `СЭС «Гордез» предоставляет гарантию на все виды обработок. Срок гарантии — от 6 месяцев 
